...
**最后更新**: 2026-04-23
**阶段**: 研究观察期（主模型可靠度已重估为偏弱，等待真实样本积累）
...
| 领域 | 覆盖 | 状态 |
|---|---|---|
| A 股分形因子模型 | 39 因子 IC-adaptive（含 MF-DFA 谱族 + asym）| ⚠️ 可运行，但当前自动可靠度为弱 |
| 美股技术分析 | 6 只（NVDA/TSLA/GOOGL/AAPL/TCOM/FUTU）| ⚠️ 可运行，当前仅 AAPL 为中，其余为弱 |
| 美股期权策略 | 分形信号映射 7 种情境，v2 已基于回测修正 | ✅ 已部署，单腿超卖反弹相对最可信 |
